...
public class NetworkAdapter extends JDialog{
    ...
    public enum MessageType { 
        ...
        /** Request to join. This message has the form "join:". */
        JOIN ("join:"), 
         ...
         * x, y: 0-based column/row indexes, v: number
         */
        JOIN_ACK ("join_ack:"), 
    ...
    public interface MessageListener {
         ...
         */
        void messageReceived(MessageType type, int x, int y, int z, int[] others, String msg);
    }
    ...
    private void parseMessage(String msg) {
        ...
        } else if(msg.startsWith(MessageType.MESSAGE.header)){
            parseSimpleMessage(MessageType.MESSAGE, msgBody(msg));
        }else if(msg.startsWith(MessageType.NAME.header)){
            parseSimpleMessage(MessageType.NAME, msgBody(msg));
        }else if(msg.startsWith(MessageType.COLOR.header))
            parseSimpleMessage(MessageType.COLOR, msgBody(msg));
        else {
    ...
    private void parseJoinMessage(String msgBody)
    {
        notifyMessage(MessageType.JOIN, msgBody);
    }
    ...
    private void parseSimpleMessage(MessageType type, String msgBody) {
        notifyMessage(type, msgBody);
    }
    ...
    public void writeJoin() {
        writeMsg(String.format("%s",MessageType.JOIN.header));
    }
    
    public void writeName(String name){
        writeMsg(String.format("%s%s", MessageType.NAME.header, name));
    }
    ...
    public void writeMessage(String msg) {
        writeMsg(String.format("%s %s",MessageType.MESSAGE.header, msg));
    }
    
    public void writeColor(String color) {
        writeMsg(String.format("%s%s",MessageType.COLOR.header, color));
    }
    
    /** Notify the listener the receipt of the given message type. */
    private void notifyMessage(MessageType type) {
        listener.messageReceived(type, 0, 0, 0, EMPTY_INT_ARRAY, null);
    }
    
    /** Notify the listener the receipt of the given message type. */
    private void notifyMessage(MessageType type, int x) {
        listener.messageReceived(type, x, 0, 0, EMPTY_INT_ARRAY, null);
    }
    
    /** Notify the listener the receipt of the given message type. */
    private void notifyMessage(MessageType type, int x, int[] others) {
        listener.messageReceived(type, x, 0, 0, others, null);
    }
    
    /** Notify the listener the receipt of the given message. */
    private void notifyMessage(MessageType type, int x, int y, int v) {
        listener.messageReceived(type, x, y, v, EMPTY_INT_ARRAY, null);
    }
    
    /** Notify the listener the receipt of the given message type. */
    private void notifyMessage(MessageType type, int x, int y, int[] others) {
        listener.messageReceived(type, x, y, 0, others, null);
    }
    
    private void notifyMessage(MessageType type, String msg) {
        listener.messageReceived(type, 0, 0, 0, EMPTY_INT_ARRAY, msg);
    }
