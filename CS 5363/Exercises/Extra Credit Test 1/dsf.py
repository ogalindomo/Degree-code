...
def observe():
    cv2.destroyAllWindows()
    video = cv2.VideoCapture('/Users/oscargalindo/Desktop/Classes/CS 5363/Exercises/Extra Credit Test 1/tracking.mp4')
    ok, frame = video.read()
    cv2.imshow("Feed",frame)
    while ok:
        ok,frame = video.read()
...
if __name__ == "__main__":
    ...
    # previous_c,previous_r = min_c,min_r
    # frame2 = f[351]
    # corr = correlate2d(f_s[351], obj_single, mode='valid')
    # motion = get_motion(frame,previous_r,previous_c)
    # motion = motion[:corr.shape[0],:corr.shape[1]]
    # m = corr*motion
    # plt.imshow(m,cmap='gray')
    # y, x = np.unravel_index(np.argmax(m), corr.shape)
    # obj = frame2[y:y+bbox[3],x:x+bbox[2],:]
    # plt.imshow(obj)
    # print(y,x)
    # observe()
